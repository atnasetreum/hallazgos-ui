...
const IcsPage = () => {
  const [data, setData] = useState<ICSData[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [filters, setFilters] = useState<IFiltersIcs>({
    name: "",
  });
    ...
    <Grid container>
      ...
      <Grid item xs={12} sm={12} md={12}>
        {isLoading ? (
          ...
          <TableIcs rows={data} getData={getData} />
        )}
      </Grid>
    </Grid>
  );
